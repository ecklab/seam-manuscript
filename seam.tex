...
\author{Julia Wapner$^1$\thanks{jwapner2@illinois.edu}, 
%Charlie Young\thanks{cdy2@illinois.edu}, 
David Dalpiaz$^2$\thanks{dalpiaz2@illinois.edu},
and Daniel J. Eck$^2$\thanks{dje13@illinois.edu} \\[1em]
1. Baseball Analytics, Baltimore Orioles \\
...
This high resolution data has ushered in a new sophisticated class of techniques for finding and exploiting players' strengths and weaknesses. Use of these techniques have affected player performance and outcomes, which is reflected but noted in the boxscore. As an example, \cite{jensen2009bayesball} showed how this data could be used to better quantify defensive ability using Baseball Information Solutions data through 2007. One of the most prominent examples of the success of this high-resolution data has been the implementation of defensive shifting based on the spray chart \citep{pettispray, marchi2019analyzing}, a plot of locations a batter has hit the ball in the past. Spray charts are used extensively by MLB organizations for fielder positioning \citep[page 271]{marchi2019analyzing}. Through the early part of the 2022 MLB season, MLB had seen infield shifts on 37\% of pitches \citep{baccellieri2022infield}. This rate of deployment demonstrates the effectiveness of the strategy.
...
Mathematical justification for the combination of these estimated spray distributions is provided. We also demonstrate that our SEAM method outperforms the empirical batter or pitcher spray chart distributions through a detailed validation study in which we hold out data from the 2021 season and determine how well $(1-\alpha)\times 100\%$ highest density regions include the batted-ball locations from the held out data. SEAM exhibits nominal marginal coverage, but is no better than the existing batter or pitcher spray charts. SEAM  outperforms batter or pitcher spray charts when we evaluate conditional coverage, indicating that our approach is better suited for predicting batted-ball locations. That being said, the SEAM confidence region is larger than those constructed from empirical batter or pitcher spray chart distributions. We then consider fixed-size confidence regions as a compromise between size and coverage. We find that the SEAM confidence region has higher average conditional coverage than its competitors when we compare fixed-size confidence regions. The size of these fixed-size regions is chosen to reflect the proportion of a baseball field that fielders could cover. Therefore, the SEAM method exhibits an important advantage over its competitors. 

We argue that the advantage that SEAM offers translates to an additional win over the course of the season. MLB organizations have repeatedly demonstrated a willingness to spend millions of dollars to obtain a comparable advantage via free agency \citep{clemens2021WAR}. The importance of winning one additional game per season can not be overstated. Had the 2011 Atlanta Braves won one additional game, they would have forced a one-game tiebreaker between them and the St. Louis Cardinals where the winner of this tiebreaker goes to the playoffs and the loser goes home. Instead, the Braves missed the playoffs altogether and the Cardinals won the World Series in historic fashion\footnote{\url{https://www.youtube.com/watch?v=46QITnnPINY&ab_channel=SecretBase}}.


\section{Setting}

Suppose that left-handed batter Joey Votto is about to face right-handed pitcher Justin Verlander, and further suppose that the fielding team desires to employ a Votto-specific shift. Both players are unique talents and they seldom face each other, this matchup has only produced two balls in play. Therefore, past matchup data is too sparse to be of much use. The fielding team could instead select fielder locations based on an estimated density constructed from Votto's spray chart or Verlander's spray chart allowed. These approaches suggest different fielder positioning schemes, see Figure~\ref{Fig:VerlanderVotto}. In this article, we propose the SEAM (synthetic estimated average matchup) method as a solution to these problems. The SEAM method provides a batter-pitcher specific spray chart density that can be used for more reliable fielder positioning. We will demonstrate that SEAM is superior to positioning strategies that are formed from exclusive use of either the batter or pitcher data. See Figure~\ref{Fig:VerlanderVotto} for the estimated SEAM spray chart density corresponding to the Joey Votto vs. Justin Verlander matchup.

\begin{figure}[t!]
\begin{center}
	\includegraphics[width = 2.5in, height = 2.5in]{VerlanderVotto1.png} 
	\includegraphics[width = 2.5in, height = 2.5in]{VerlanderVottoSEAM.png}\\
	\includegraphics[width = 2.5in, height = 2.5in]{VerlanderVotto2.png}
	\includegraphics[width = 2.5in, height = 2.5in]{VerlanderVotto3.png}	
\end{center}
...
The estimators \eqref{synth-est} are biased estimators for $f$. However, they have the potential to reduce MSE. One obvious case is when, for all
...
However, the highest density region for SEAM is slightly larger than its competitors. As an example, the level $\alpha = 0.10$ highest density region for SEAM includes 34\% of grids forming the rectangle that contains the playing field while similar highest density regions corresponding to the empirical batter and pitcher spray distributions include, respectively, 33.4\% and 33\% of such grids. 

We will now consider a different type of confidence region which will balance these conflicting conclusions with size and coverage. This region is constructed using the $n_g$ grids containing the highest probable batted ball locations as estimated by each method. Calculating average conditional coverages for fixed-size regions is an important consideration for fielding placement due to the physical constraint on how much of the baseball field defenders can reasonably cover. We considered [1500, 2500] as a range for the number of grids. This range corresponds to fixed-size regions that cover approximately [50\%,\;80\%] of fair territory of a baseball field that is 115,000 square feet. 

We then assess the average conditional coverage of these fixed-size regions. What we find is that the SEAM fixed-size region has higher average conditional coverage than its competitors which ignore matchup considerations. This is despite the fact that the cumulative probability mass of the SEAM fixed-size region is lower than its competitors. The results are in %Table~\ref{Tab:fixed-size}, and 
Figure~\ref{Fig:fixed-size}. Note that were 129937 baseballs put into play in 2021. Thus, an increase in coverage by 1\% roughly corresponds to an increase of fielding 40 outs per team, and a difference of 40 outs corresponds to about one and half baseball games. It is reasonable to expect that any baseball team that implements our SEAM method could win an additional game over the course of the season. As stated in the Introduction, MLB organizations have repeatedly demonstrated a willingness to spend millions of dollars to obtain a comparable advantage via free agency \citep{clemens2021WAR}, and an additional win could be all that is needed to have a chance at competing for a championship.

%The differences between the methods in Figure~\ref{Fig:fixed-size} are slight, but very important since a 1\% is potentially worth millions of dollars. 
We now explore statistical significant differences between the methods depicted in Figure~\ref{Fig:fixed-size}. Individual coverages for particular matchups are quite variable. Therefore, point-wise differences between methods are not statistically significant. However, a linear regression model with coverage as the response, and method and the number of grids as predictors, reveals that the SEAM main effect is statistically significantly greater than those corresponding to competing methods. But classical tests of regression coefficients are dubious because diagnostic plots reveal modest violations of modeling assumptions. Violations of modeling assumptions were expected since the response variable lives in $[0,1]$. A nonparametric bootstrap of ordinary least squares (OLS) estimates was then performed with bootstrap sample size set at 10000. This analysis agreed with the original regression analysis, the proportion of bootstrap iterates for which the estimated coefficient corresponding to the SEAM method was exceeded by estimated coefficient corresponding to the empirical batter and empirical pitcher spray chart distributions was, respectively, 0 and 0.0147. More comprehensive results are depicted in Figure~\ref{Fig:boot}. This analysis suggests that the advantage offered by the SEAM method is indeed statistically significant. Note that a bootstrap analysis revealed that the regression model presented here was preferred over a larger model which included a separate slope coefficient for the empirical pitching method.
...
\end{center}	
\caption{Average conditional coverage of fixed-size confidence regions for SEAM, the empirical batter spray distribution, and the empirical pitcher spray distribution. The fixed-size regions are constructed from the $n_g$ most probable grids according to each method. }
...
\begin{figure}[t!]
\begin{center}
\includegraphics[width=0.85\textwidth]{bootstrap_density.pdf}	
\end{center}	
\caption{Nonparametric bootstrap investigation of the statistical significance of the increased coverage offered by our SEAM method. The distributions depicted are differences between OLS estimated coefficients between the empirical batter (left panel) and pitcher (right panel) spray chart distributions and the SEAM method. Negative values indicate preference for the SEAM method. The proportion of iterates that are less than 0 is 0 (left panel) and 0.0147 (right panel).}
\label{Fig:boot}
\end{figure}
